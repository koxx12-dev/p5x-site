...
export function AwarenessDisplay(props: AwarenessDisplayProps) {
  ...
  const a = createMemo(() => typeof ourProps.tier === "string" ? ourProps.tier : `a${ourProps.tier}` as Awareness);

  return (
    <Show when={getAwarenessAsset(a())} fallback={
      <span class="font-bold text-white">
        MISSING AWARENESS ASSET {a()}
      </span>
    }>
      {(src) => (
        <ResponsiveImage
