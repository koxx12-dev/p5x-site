...
import { useDefaultTranslator, useRegion } from './i18n';
...
const tG = useDefaultTranslator(i18nDict);
		...
		skill: thief.skill?.map((skill) => ({
			...
			description:
				typeof skill.description === 'string'
					? t(skill.description) || skill.description
					: skill.description,
		})),
	};
}
...
export function useThief(
	thiefId: MaybeAccessor<string>,
): Accessor<ReturnType<typeof translateThief> | undefined> {
	...
	return createMemo(() => {
		...
		if (
			thief === undefined ||
			(thief.available_regions !== undefined &&
				!thief.available_regions.includes(region()))
		) {
			return undefined;
		}
